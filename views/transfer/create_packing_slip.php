...
// 1. Query untuk mengambil data Transfer (Header)
...
if (!$transaction) {
    die('<div class="alert alert-danger container my-4">Error: Data Transfer Gudang tidak ditemukan atau ID tidak valid.</div>');
}

// --- LOGIKA Pengecekan Packing Slip yang Sudah Ada ---
$ps_check_query = "SELECT * FROM transfer_packing_slips WHERE transfer_id = ?";
$stmt_ps_check = $conn->prepare($ps_check_query);
$stmt_ps_check->bind_param("i", $transfer_id);
...
if ($packing_slip_data) {
    ...
    $last_ps_query = "SELECT packing_slip_number FROM transfer_packing_slips ORDER BY id DESC LIMIT 1";
    ...
    if ($last_number && preg_match('/(\d+)\//', $last_number, $matches)) {
        ...
        $next_sequence = 1; // Angka awal jika belum ada data
    }
    ...
    $current_year = date('Y');
    $site_code = 'TAG-JKT'; // Penanda khusus Transfer
    ...
    $default_koli = '';           
    $default_recipient = '';      
    $default_subject = 'PENGIRIMAN BARANG MUTASI';        
    $default_location = '';       
    $default_notes = '';          
...
$stmt_details->bind_param("i", $transfer_id);
$stmt_details->execute();
$items_res = $stmt_details->get_result();

while ($row = $items_res->fetch_assoc()) {
    $ti_id = $row['transfer_item_id'];
...
<head>
    ...
    <title><?= $is_edit_mode ? 'Review/Ubah' : 'Buat' ?> Packing Slip - Mutasi #<?= htmlspecialchars($transaction['transfer_no']) ?></title>
...
<div class="container my-4">
    <div class="content-header mb-4 d-flex justify-content-between align-items-center">
        <h1 class="fw-bold"><?= $is_edit_mode ? 'Review & Ubah' : 'Buat' ?> Packing Slip TAG</h1>
        <a href="detail.php?id=<?= $transfer_id ?>" class="btn btn-secondary rounded-pill px-3">
            <i class="bi bi-arrow-left me-2"></i>Kembali ke Detail Mutasi
        </a>
    </div>
    ...
    <div class="card shadow-lg mb-4 border-primary">
        <div class="card-header bg-primary text-white">
            <h5 class="mb-0">Data Referensi (Mutasi Antar Gudang)</h5>
            <small>Surat Jalan #<?= htmlspecialchars($transaction['transfer_no']) ?></small>
        </div>
        <div class="card-body bg-light">
            <div class="row">
                <div class="col-md-6">
                    <strong>No. Transfer:</strong> <?= htmlspecialchars($transaction['transfer_no']) ?><br>
                </div>
                <div class="col-md-6">
                    <strong>Tanggal Dibuat:</strong> <?= date('d-m-Y H:i', strtotime($transaction['transaction_date'])) ?><br>
                    <strong>Catatan Mutasi:</strong> <?= htmlspecialchars($transaction['notes']) ?: 'Tidak ada' ?>
                </div>
            </div>
        </div>
    </div>
    ...
    <form id="packingSlipForm" method="POST" action="<?= $form_action ?>">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" class="form-control" id="koli_count" name="koli_count" 
                               ...
                               placeholder="Total koli fisik TAG">
                    </div>
                </div>

                <div class="row g-3 mt-2">
                    <div class="col-md-4">
                        <label for="recipient" class="form-label required-field">To / Kepada</label>
                        <input type="text" class="form-control" id="recipient" name="recipient" 
                               value="<?= $is_edit_mode ? htmlspecialchars($default_recipient) : '' ?>" required 
                               placeholder="Contoh: Ekspedisi JNE / Sopir">
                    </div>
                    ...
                    <div class="col-md-4">
                        <label for="location" class="form-label required-field">Location (Gudang Tujuan)</label>
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-white">
                <h5 class="mb-0 fw-bold text-dark"><i class="bi bi-box-seam me-2"></i>Detail Produk Mutasi</h5>
            </div>
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-striped align-middle">
                        ...
                        <tbody>
                                ...
                                <?php $item_index = 0; foreach ($details as $detail): ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <span class="badge bg-primary fs-6"><?= htmlspecialchars($detail['quantity']) ?></span>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="text-center text-muted">Tidak ada produk dalam mutasi ini.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
        ...
        <div class="text-center mb-5">
            ...
            <?php endif; ?>
            <a href="detail.php?id=<?= $transfer_id ?>" class="btn btn-outline-secondary btn-lg rounded-pill px-5">
